...
`AdaptiveVariantUpdate.reason` is `'initial'` for the first selection, `'automatic'` for later ABR changes, and `'manual'` when `setAdaptiveQuality()` selects a specific variant.
